...
#ifndef _RECOVERED_HPP__
#define _RECOVERED_HPP__
...
using namespace std;
    ...
    class recovered{
        public:
            ...
            recovered(double initial_population){
            ...
            void external_transition(TIME e, typename make_message_bags<input_ports>::type mbs) {
                ...
                // R'  = ((yi*I)+(ya*A)+(yh*H))
                state.population_delta = ((yi*state.symptomatic_infective)+(ya*state.asymptomatic_infective)+(yh*state.quarantined_infective))*dt;
                state.report_queued = true;
            }
            ...
            friend std::ostringstream& operator<<(std::ostringstream& os, const typename recovered<TIME>::state_type& i) {
                 os.precision(2);
                os << fixed;
                os << "{\"recovered\":" << i.population+i.population_delta << "}";
            return os;
            }
        };


#endif // _RECOVERED_HPP__
