...
#ifndef _QUARANTINED_SUSCEPTIBLE__
#define _QUARANTINED_SUSCEPTIBLE__
...
using namespace std;
    ...
    class quarantined_susceptible{
        public:
            ...
            // default constructor
            quarantined_susceptible() noexcept{
              assert(false && "default constructor is not available");
            }
            quarantined_susceptible(double initial_population){
            ...
            void external_transition(TIME e, typename make_message_bags<input_ports>::type mbs) {
                ...
                state.population_delta = (((1-b)*c*q*state.susceptible*(state.symptomatic_infective+state.asymptomatic_infective))-(l*state.population))*dt;
                state.report_queued = true;
            }
            ...
            friend std::ostringstream& operator<<(std::ostringstream& os, const typename quarantined_susceptible<TIME>::state_type& i) {
                 os.precision(2);
                os << fixed;
                os << "{\"quarantined_susceptible\":" << i.population+i.population_delta << "}";
            return os;
            }
        };


#endif // _QUARANTINED_SUSCEPTIBLE__
