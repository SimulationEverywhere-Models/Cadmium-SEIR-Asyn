...
#ifndef _QUARANTINED_INFECTIVE__
#define _QUARANTINED_INFECTIVE__
...
using namespace std;
    ...
    class quarantined_infective{
        public:
            ...
            quarantined_infective(double initial_population){
            ...
            void external_transition(TIME e, typename make_message_bags<input_ports>::type mbs) {
                ...
                // H'  = (di*I)+(dq*Eq)-((a+yh)*H)
                state.population_delta = ((di*state.symptomatic_infective)+(dq*state.quarantined_exposed)-((a+yh)*state.population))*dt;
                state.report_queued = true;
            }
            ...
            friend std::ostringstream& operator<<(std::ostringstream& os, const typename quarantined_infective<TIME>::state_type& i) {
                 os.precision(2);
                os << fixed;
                os << "{\"quarantined_infected\":" << i.population+i.population_delta << "}";
            return os;
            }
        };


#endif // _QUARANTINED_INFECTIVE__
