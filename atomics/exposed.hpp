...
using namespace std;
    ...
    class exposed{
        public:
            ...
            exposed(double initial_population){
            ...
            void external_transition(TIME e, typename make_message_bags<input_ports>::type mbs) {
                ...
                // E'  = (b*c*(1-q)*susceptible*(symptomatic_infective+asymptomatic_infective))-(e*exposed)
                state.population_delta = ((b*c*(1-q)*state.susceptible*(state.symptomatic_infective+state.asymptomatic_infective))-(e*state.population))*dt;
                state.report_queued = true;
            }
            ...
            friend std::ostringstream& operator<<(std::ostringstream& os, const typename exposed<TIME>::state_type& i) {
                 os.precision(2);
                os << fixed;
                os << "{\"exposed\":" << i.population+i.population_delta << "}";
            return os;
            }
        };
