...
#ifndef _SYMPTOMATIC_INFECTIVE_HPP__
#define _SYMPTOMATIC_INFECTIVE_HPP__
...
using namespace std;
    ...
    class symptomatic_infective{
        public:
            ...
            // default constructor
            symptomatic_infective() noexcept{
              assert(false && "default constructor is not available");
            }
            symptomatic_infective(double initial_population){
            ...
            void external_transition(TIME t, typename make_message_bags<input_ports>::type mbs) {
                ...
                state.population_delta = (
                    -I_to_H (di, state.population)
                    -I_to_R (yi, state.population)
                    -I_to_D (a, state.population)
                    +E_to_I (e, n, state.E)
                )*dt;
                state.report_queued = true;
            }
            ...
            friend std::ostringstream& operator<<(std::ostringstream& os, const typename symptomatic_infective<TIME>::state_type& i) {
                 os.precision(2);
                os << fixed;
                os << "{\"symptomatic_infective\":" << i.population+i.population_delta << "}";
            return os;
            }
        };


#endif // _SYMPTOMATIC_INFECTIVE_HPP__
